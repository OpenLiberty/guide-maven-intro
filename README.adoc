...
If you haven't already installed Maven, https://maven.apache.org/download.cgi[Download the binary zip or tar.gz file^].
...
element and [hotspot=modelVersion file=0]`<modelversion/>`, which is always set to `4.0.0`. 
...
In the [hotspot=liberty-maven-plugin file=0]`liberty-maven-plugin` plugin section, you can add a
[hotspot=configuration file=0]`<configuration>` element to specify Liberty configuration details.
For example, you can add the
[hotspot=configuration file=0]`<serverName/>` field to define the name of the Liberty server that Maven creates.
The default value is `defaultServer`.
You can uncomment out the [hotspot=configuration file=0]`<configuration>` element to use `guideServer` as the name
for the Liberty server instance.
