...
both the server configuration ([hotspot file=2]`server.xml`) file and will be used in the test class that
...
and then development mode can run any integration test classes (classes with names that end in `IT`) that it 
finds.
