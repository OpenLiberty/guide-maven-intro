...
You specified the value for [hotspot=serverName file=0]`<serverName/>` to be `guideServer`.
If the [hotspot=serverName file=0]`<serverName/>` field is not provided, the default value is `defaultServer`.
