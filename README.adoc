...
element and [hotspot=modelVersion file=0]`<modelversion/>`, which is always set to `4.0.0`. This is the minimum you technically need for the POM to be valid. 
...
places throughout the application. For example, the [hotspot=test-server-port file=0]`default.http.port` value is used in 
...
In the [hotspot=liberty-maven-plugin file=0]`liberty-maven-plugin` plugin section, you can add the `<serverName/>` 
...
Java test program as a series of system properties, resolving the [hotspot=liberty-test-port file=1]`http.port` 
