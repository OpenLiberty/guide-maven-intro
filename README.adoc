...
:page-seo-title: Build and test a web app with Maven and Open Liberty
:page-seo-description: A tutorial on how to build and test a simple web application using Maven and Open Liberty
...
element and [hotspot=modelVersion file=0]`<modelversion/>`, which is always set to `4.0.0`. The [hotspot=parent file=0]`<parent/>` section 
...
places throughout the application. For example, the [hotspot=test-server-port file=0]`testServerHttpPort` value is used in 
...
In the [hotspot=liberty-maven-plugin file=0]`liberty-maven-plugin` plugin section, the [hotspot=server-name file=0]`<serverName/>` 
field defines the name of the Liberty server that Maven creates. The [hotspot=bootstrap-properties file=0]`<bootstrapProperties/>` 
section provides the set of variables that the [hotspot=http-port file=2]`server.xml` references. The 
[hotspot=strip-version file=0]`<stripVersion/>` field removes the version number from the name of the application 
...
Java test program as a series of system properties, resolving the [hotspot=liberty-test-port file=1]`liberty.test.port` 
...
The Apache [hotspot=commons-httpclient file=1]`commons-httpclient` and [hotspot=junit file=1]`junit` 
