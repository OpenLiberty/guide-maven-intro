...
Before you can build the project, you create the `pom.xml` file. The POM file starts with a root `<project>` element that is followed by the POM version, the project coordinates, and the parent POM to this project:
...
- The project coordinates, which consists of a group ID ( `groupId`), an artifact ID (`artifactId`), a version (`version`) , and the packaging for the POM (`packaging`). For a web application project, the value of the `packaging` field is `war` so that the project output artifact is a WAR file.
...
[source,java]
----
include::finish/src/test/java/io/openliberty/guides/hello/ApplicationTest.java[**;!comment]
----
