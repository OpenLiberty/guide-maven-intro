...
Gradle is another build tool similar to Maven. However, Gradle has a few key differences from Maven.
...
Benefits of Maven over Gradle include a greater variety of functions and support available through its larger plugin library. 
Maven's integration with IDEs and other tools is also more developed than Gradle's.
