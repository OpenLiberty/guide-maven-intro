...
The [hotspot]`pom.xml` file starts with a root [hotspot=2]`<project/>` 
element and [hotspot=3]`<modelversion/>`, which is always set to `4.0.0`. The [hotspot=5-9]`<parent/>` section 
...
* **Properties** ([hotspot=16-27]`<properties/>`): Any properties for the project go here, 
...
* **Dependencies** ([hotspot=28-68]`<dependencies/>`): Any Java dependencies that are 
required for compiling, testing, and running the application are listed here.
* **Build plugins** ([hotspot=69-134]`<build/>`): Maven is modular and each of its 
...
[hotspot=12]`artifactId` gives a name to the web application project, which is used to 
name the output files that are generated by the build (e.g. the WAR file) and the Open 
Liberty server that is created. You'll notice that other fields in the [hotspot]`pom.xml` 
file use variables that are resolved by the [hotspot=12]`artifactId` field. This is so 
that you can update the name of the sample application, including files generated by Maven, 
in a single place in the [hotspot]`pom.xml` file. The value of the [hotspot=13]`packaging` 
field is `war` so that the project output artifact is a WAR file.

The first four properties in the properties section of the project, just define the encoding ([hotspot=17-18]`UTF-8`) 
and version of Java ([hotspot=19-20]`Java 8`) that Maven uses to compile the application 
source code.

The next four properties provide a single place to specify values that are used in multiple 
places throughout the application. For example, the [hotspot=22]`testServerHttpPort` value is used in 
...
The `HelloServlet.java` class depends on [hotspot=62-67]`javax.servlet-api` to compile. 
Maven will download this dependency from the Maven Central repository using the 
[hotspot=63]`groupId`, [hotspot=64]`artifactId`, and [hotspot=65]`version` details that 
you provide here. The dependency is set to [hotspot=66]`provided`, which means that the 
API is in the server runtime and doesn't need to be packaged by the application.

The [hotspot=69-134]`<build/>` section gives details of the two plugins that Maven uses 
...
In the [hotspot=79-108]`liberty-maven-plugin` plugin section, the [hotspot=89]`<serverName/>` 
field defines the name of the Liberty server that Maven creates. The [hotspot=94-98]`<bootstrapProperties/>` 
section provides the set of variables that the `server.xml` references. The 
[hotspot=90]`<stripVersion/>` field removes the version number from the name of the application 
