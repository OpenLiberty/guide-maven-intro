...
Using a build tool like Maven is beneficial as it handles the downloading and installation of all dependencies.
...
Maven has some differences and benefits over another build tool called Gradle. 
Maven defines its builds using `.xml` files in a hierarchical dependency tree. 
As an older tool Maven also has greater integrations with IDEs. 
Maven also has an extensive plugin library which offers a variety of ways to quickly customize your build.
This makes Maven a good choice for smaller, quicker projects. 
