...
One benefit of using a build tool like Maven is that you can define the details of the project and any dependencies it has, and then Maven automatically downloads and installs the dependencies.
...
Choosing a build tool often comes down to personal preference, but you might choose to use Maven for several reasons. 
Maven defines its builds using XML, which is probably familiar to you already. 
As a mature, commonly used build tool, Maven probably integrates with whichever IDE you prefer to use. 
Maven also has an extensive plugin library which offers a variety of ways to quickly customize your build. 
Maven can be a good choice if your team is already familiar with it. 

You will create a Maven build definition file called a `pom.xml` file, which stands for 
project object model, and use it to build your web application. You will then create a simple, 
automated test and configure Maven to run automatically run the test.
...
The web application that you will use in this guide has been provided for you under 
the `start` directory. This application uses a standard Maven directory structure, 
eliminating the need to customize the pom.xml file so that Maven understands your project layout.
...
to run a set of predefined tests. You can write tests for the individual units of code outside of a 
