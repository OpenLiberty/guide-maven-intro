...
== Why Use Maven Toolchains with Liberty?

Historically, the `liberty-maven-plugin` used the same JDK that was running the Maven process. However, modern Java development often requires building or testing applications on a specific Java version (e.g., Java 8 or 11) while Maven itself might be running on a different version (e.g., Java 17 or 21).

Starting with version **3.12.0**, the `liberty-maven-plugin` introduced support for **Maven Toolchains** to address these needs:

* **Decoupling environments:** It allows the Liberty server to run on a JDK that is different from the one running Maven.
* **Reproducibility:** It ensures that every developer on a team and every CI/CD pipeline uses the exact same JDK vendor and version, regardless of their local `JAVA_HOME` settings.
* **Streamlined Testing:** It simplifies testing applications against multiple Java versions without needing to manually switch environment variables between runs.

---

== Supporting Maven Toolchains in Liberty

The `liberty-maven-plugin` supports Maven Toolchains starting from version **3.12.0**.
...
<plugin>
    ...
    <version>3.12.0</version> <configuration>
        <jdkToolchain>
            <version>17</version>
            <vendor>temurin</vendor>
        </jdkToolchain>
...
Now you can use a different version of Java as JAVA_HOME, and liberty will run using Java 17.
Run your project using the following command to see the toolchain in action:
...
To confirm that the correct Java version is being used, check the `messages.log` file located in the `target/liberty/wlp/usr/servers/defaultServer/logs/` directory.

The header of the log file will indicate the `java.home` and `java.version` provided by the toolchain:
...
os = Mac OS X (15.6.1; aarch64) (en_US)
process = 13271@local
********************************************************************************
----
