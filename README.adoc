...
The [hotspot=1-20 file=0]`pom.xml` file provided for you in the `start` directory starts 
with a root [hotspot=2-11 file=0]`<project />` element and [hotspot=3 file=0]`<modelversion />`, 
which is always set to `4.0.0`. The [hotspot=4-8]`<parent />` section of the POM specifies 
that Maven will use the https://github.com/WASdev/ci.maven/blob/master/liberty-maven-app-parent/pom.xml[Liberty Maven parent POM] 
...
[hotspot=8 file=0]`</parent>` and [hotspot=11 file=0]`</project>` tags. Unless specified 
otherwise, add each of the sections of XML code beneath the previous one. The 
[hotspot=11 file=0]`</project>` tag must be the last thing in the `pom.xml` file.
...
[role="code_command hotspot=9-12", subs="quotes"]
----
...
[role="code_command hotspot=13-27", subs="quotes"]
----
