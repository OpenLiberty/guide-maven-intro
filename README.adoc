...
in a single place in the [hotspot]`pom.xml` file. The value of the [hotspot=13 file=0]`packaging` 
...
section provides the set of variables that the [hotspot=6-7 file=2]`server.xml` references. The 
...
Maven is configured to run the integration test using the [hotspot=110-133 file=1]`maven-failsafe-plugin`.
