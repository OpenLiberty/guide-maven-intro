...
Liberty server that is created. You'll notice that other fields in the [hotspot=1-137]`pom.xml` 
...
in a single place in the [hotspot=1-137]`pom.xml` file. The value of the [hotspot=12]`packaging` 
...
[role="code_command hotspot=66-136", subs="quotes"]
----
Configure the Maven plugins that build this project.
`src/pom.xml`
----

The [hotspot=66-136]`<build />` section gives details of the two plugins that Maven uses 
...
In the [hotspot=76-110]`liberty-maven-plugin` plugin section, the [hotspot=87]`<serverName />` 
field defines the name of the Liberty server that Maven creates. The [hotspot=92-96]`<bootstrapProperties />` 
section provides the set of variables that the `server.xml` references. The 
[hotspot=88]`<stripVersion />` field removes the version number from the name of the application 
...
[role="code_command hotspot=10-37", subs="quotes"]
----
...
Maven is configured to run the integration test using the [hotspot=111-134 file=1]`maven-failsafe-plugin`.
The [hotspot=125-130 file=1]`<systemPropertyVariables />` section defines some variables 
...
test class, it is better to specify it in a single place like the Maven [hotspot=1-137 file=1]`pom.xml` 
file because this information is also used by other files in the project. The 
[hotspot=125-130 file=1]`<systemPropertyVariables />` section passes these details to the 
Java test program as a series of system properties, resolving the [hotspot=126-128 file=1]`liberty.test.port` 
and [hotspot=129 file=1]`war.name` variables.
...
The following lines in the [hotspot=30-32 file=0]`Endpoint.java` test class uses these 
system variables to build up the URL of the application.

In the test class, after defining how to build the application URL, the [hotspot=23 file=0]`@Test` 
annotation indicates the start of the test method.

In the [hotspot=25-34 file=0]`try` block of the test method, an HTTP `GET` request to the 
...
the [hotspot=1-137 file=1]`pom.xml` to include these dependencies.

The Apache [hotspot=33-38 file=1]`commons-httpclient` and [hotspot=39-44 file=1]`junit` 
dependencies are needed to compile and run the integration test [hotspot=10-37 file=0]`EndpointIT` 
